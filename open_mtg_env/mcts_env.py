...
# Performs MCTS over env class, rather than game class
...
class Node:
    ...
    def __init__(self, move=None, parent=None, state=None):
        ...
        self.wins = 0
        self.visits = 0
        self.untried_moves = copy.copy(state.move_strings)  # future child nodes
        self.player_just_moved = state.game.player_just_moved  # the only part of the state that the Node needs later
...
def uct(rootstate, itermax, verbose=False):
    ...
    for i in range(itermax):
        ...
        if len(state.game.players[k].deck) > 0:
            while len(state.game.players[k].deck) > 0 and state.game.players[k].deck[-1].deck_location_known:
                indexed_cards_in_deck.append(state.game.players[k].deck.pop())
        for indexed_card in indexed_cards_in_deck:
            state.game.players[k].deck.append(indexed_card)
        # and "imagine" a scenario for the opponent - this assumes knowledge of opponent decklist!

        state.game.players[k].shuffle_deck()
        opponent = state.game.players[1 - k]
        ...
        moves = state.move_strings
        done = state.game.is_over()
        while not done:  # while state is non-terminal
        ...
        while node is not None:  # backpropagate from the expanded node and work back to the root node
            ...
            node.update(state.game.get_results(node.player_just_moved.index))
            node = node.parent
